...
void connet_display(uint8_t flag)
{
    if((flag & 0x80) == 0x80)
    {
        ...
        lcd_show_string(0, 17, lcd_self.width, 16, 16, lcd_buff, BLUE);
        sprintf(lcd_buff, "psw:%s",DEFAULT_PWD);
        lcd_show_string(0, 33, lcd_self.width, 16, 16, lcd_buff, BLUE);
        lcd_show_string(0, 49, lcd_self.width, 16, 16, "BOOT:Send data", MAGENTA);
    }
    else if ((flag & 0x04) == 0x04)
    {
        lcd_show_string(0, 65, lcd_self.width, 16, 16, "wifi connecting......", BLUE);
    }
    else if ((flag & 0x02) == 0x02)
    {
        lcd_show_string(0, 65, lcd_self.width, 16, 16, "wifi connecting fail", BLUE);
    }
